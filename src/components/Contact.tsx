...
export default function ContactSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(
        leftContentRef.current?.children as HTMLCollection,
        { opacity: 0, y: 30 },
        {
          opacity: 1,
          y: 0,
      ...
      // 2. Right Column (Form) Reveal
      gsap.fromTo(
        rightContentRef.current?.children as HTMLCollection,
        { opacity: 0, y: 30 },
        {
          opacity: 1,
          y: 0,
  ...
  return (
    <section 
      id="contact" 
      ref={sectionRef} 
      className="relative min-h-screen bg-[#E5E1D8] text-[#1A1A1A] py-24 px-6 md:px-12 xl:px-20 overflow-hidden rounded-t-[2.5rem] z-30"
    >
      <div className="max-w-350 mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 relative z-10">
      ...
      {/* 
        =========================================
        BOTTOM CORNER LAND IMAGE REPLACEMENT
        =========================================
      */}
      <div 
        ref={landImageRef}
        className="absolute bottom-[-5%] right-[-5%] w-75 md:w-112.5 lg:w-137.5 opacity-80 pointer-events-none z-0 mix-blend-multiply"
      >
        {/* Replace this SVG/Div with your transparent PNG of the land/topography */}
        <div className="relative w-full aspect-square">
           <Image 
             src="/transparent-land.png" // Put your land image here
             alt="Topography" 
             fill 
             className="object-contain object-bottom right-0"
           />
           
           {/* Placeholder if you don't have the image yet */}
           <div className="absolute inset-0 bg-neutral-400/20 rounded-full blur-3xl" />
        </div>
      </div>
